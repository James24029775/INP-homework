...
void DecideService(char *readBuf, char *username, char *pwd, int n, int *type, int *cnt);
...
int main(int argc, char *argv[]){
    ...
    for( ; ; ){
        ...
        *pconnsocke = connfd;
        ...
        pthread_create(&tid, NULL, MessageBox, (void*) pconnsocke);
    }
...
void *MessageBox(void *arg){
    ssize_t n;
    char readBuf[MAXLINE], writeBuf[MAXLINE], tmp[K], msg_t[MAXLINE];
    char *order, *username, *pwd, *msg;
    int sockfd, type=-1, cnt=0, userIndex=-1, userIndexForReg=-1, userIndexForMsg=-1, userIndexForRec=-1;
    ...
    strcpy(writeBuf, WELCOME);
    writen(sockfd, writeBuf, MAXLINE);
...
again:
    while (true)
    {
        ...
        // Deal with "%"
        bzero(&writeBuf, MAXLINE);
        strcpy(writeBuf, SHOWME);
        writen(sockfd, writeBuf, MAXLINE);
        
        // Deal with cases
        bzero(&readBuf, MAXLINE);
        if ((n = read(sockfd, readBuf, MAXLINE)) < 0) break;
                ...
                case 1:
                    if (loginOrNot[userIndex]){
                        ...
                        if (loginOrNot[userIndex]){
                            ...
                            strcpy(tmp, "Username or password is incorrect.\n");
                        }
