...
int MessageBox(int sockfd){
    ...
    int type=-1, cnt=0, userIndex=-1, userIndexForReg=-1, userIndexForMsg=-1, userIndexForRec=-1;
    bool isTrueForm=true;
    // Deal with WELCOME
    // sendMessage(writeBuf, WELCOME, sockfd);
...
again:
    while (true)
    {
        ...
        // Deal with "%"
        // sendMessage(&writeBuf, SHOWME, sockfd);
        bzero(&writeBuf, MAXLINE);
        ...
        // Decide service
        // DecideService(readBuf, username, pwd, n, &type, &cnt);
        readBuf[n-1] = '\0';
