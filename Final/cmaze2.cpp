...
str_cli(FILE *fp, int sockudp, const SA *TAserv, socklen_t servlen)
{
    ...
    VVI distance(h, vector<int>(w));
    VPI parent(h, vector<PI>(w));
    ...
    while(1)
    {
        if (first){
            ...
            sendto(sockudp, sendline, strlen(sendline), 0, TAserv, servlen);
            sendto(sockudp, sendline, strlen(sendline), 0, TAserv, servlen);
            first = false;
        }
        ...
        if (recvline[0] == '#'){
            for (int i = 0 ; i < h ; i++)
            {
                for (int j = 0 ; j < w ; j++)
                {   
                    cout << i << ' ' << j << ' ' << recvline[i*w+j] << endl;
                    map[i][j] = recvline[i*w+j];
                    
                    if (map[i][j] == '*') SS = PI(i, j);
                    if (map[i][j] == 'E') EE = PI(i, j);
                }
            }
            ...
            PI cur = SS, last;
            q.push(cur);
            distance[cur.first][cur.second] = 0;
            parent[cur.first][cur.second] = SS;
            ...
            while(!q.empty())
            {
                ...
                for(int i = 0 ; i < 4 ; i++)
                {
                    ...
                    if (map[nextx][nexty] == '#' || distance[nextx][nexty] != -1) continue;
                    
                    if (nextx == EE.first && nexty == EE.second){
            ...
            while(!stk.empty()){
                cur = stk.top();
                stk.pop();
                ddx = cur.first - last.first;
                ddy = cur.second - last.second;
                if (ddx == 0 && ddy == 1) tmp = "D\n";
                if (ddx == 0 && ddy == -1) tmp = "A\n";
                if (ddx == 1 && ddy == 0) tmp = "S\n";
                if (ddx == -1 && ddy == 0) tmp = "W\n";
